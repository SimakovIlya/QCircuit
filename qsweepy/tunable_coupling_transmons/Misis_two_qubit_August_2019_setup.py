...
class hardware_setup():
    ...
    def open_devices(self):
        ...
        # set_frequency -> выставляет частоту (в Гц)
        # set_power -> выставляет мощность (в дБм)
        ...
        # set_bandwidth(res_bw) задаёт ширину полосы частот
        #set_bandwidth_video(video_bw) обратное время измерения
        #measure() -- возвращает словарь {'Power': [...]}, в этом списке находятся измеренные мощности как функция частоты
        #get_points() -- отда>т список частотб в котором будет мериться мощность когда вызовут measure
        #set_xlim(fmin, fmax) -- выставляет диапазон частот анализатора спектра
        #set_nop(nop) -- выставляет число точек по частоте
        self.hdawg = Zurich_HDAWG1808(self.device_settings['hdawg_address'])
        # set_waveform(waveform, channel)
        # set_nop задает число точек в вейвформе (она здесь всегда 200000)
        # set_clock(clock) задаёт частоту дискретизации (она здесь 1 ГГц) 
        # waveform --  массив значений напржения как функция времени в дискретах (здесь это нс)  
        self.dummy_awg = dummy_awg.DummyAWG(channels=1)
        self.coil_device = self.hdawg

        # set_nop(nop) число точек после прихода триггера которое снимается
        # set_nums(nums) задаёт число повторений (обычно 10000, но для симулятора жостко)
        # measure() {'Voltage': ndarray(nums x nop)
        #            'avg_cov0': voltage@cov0
        #            'avg_cov1': 
        #            'avg_cov2': 
        #            'avg_cov3': }
